...
{    public class Spline
    {
        struct float9
        {
            ...
            public (double x, double y, double z, double w) Left
            {
                get => (a, b, c, d);
                set
                {
            ...
            public (double x, double y, double z, double w) Right
            {
                get => (e, f, g, h);
                set
                {
        ...
        public Spline(params (float time, float value)[] values)
        {
            ...
            var trails = new List<((double x, double y, double z, double w) left, double right)>();
            ...
            // ç≈å„ÇÃâ¡ë¨ÇÕ0
            var trail = (left: (x: 6.0 * values[^1].time, y: 2.0, z: 0.0, w: 0.0), right: 0.0);
            ...
            for (int i = 0; i < values.Length - 1; ++i)
            {
                ...
                var matrix = new Matrix4x4()
                {
                    ...
                    m20 = (float)trail.left.x,
                    m21 = (float)trail.left.y,
                    m22 = (float)trail.left.z,
                    m23 = (float)trail.left.w,

                    m30 = (float)trail2.left.x,
                    m31 = (float)trail2.left.y,
                    m32 = (float)trail2.left.z,
                    m33 = (float)trail2.left.w
                };

                var right = new Vector4();
                right.x = values[index].value;
                right.y = values[index - 1].value;
                right.z = (float)trail.right;
                right.w = (float)trail2.right;
                Debug.Log(right);

                var inversedMatrix = matrix.inverse;
                var interval = new CubicFunction(
                    Vector4.Dot(inversedMatrix.GetRow(0), right),
                    Vector4.Dot(inversedMatrix.GetRow(1), right),
                    Vector4.Dot(inversedMatrix.GetRow(2), right),
                    Vector4.Dot(inversedMatrix.GetRow(3), right));
                ...
                trail = ((6d * startTime, 2d, 0d, 0d), interval.Acceleration(startTime));
                trail2 = ((3d * startTime * startTime, 2d * startTime, 1d, 0f), interval.Velocity(startTime));
                Debug.Log(matrix);
            ...
            for (int i = 0; i < intervals.Length - 1; i++)
            {
               ...
               // UnityEngine.Assertions.Assert.AreApproximatelyEqual(a.Evaluate(t), b.Evaluate(t), t.ToString());
               // UnityEngine.Assertions.Assert.AreApproximatelyEqual(a.Acceleration(t), b.Acceleration(t), t.ToString());
               // UnityEngine.Assertions.Assert.AreApproximatelyEqual(a.Velocity(t), b.Velocity(t), t.ToString());
            }
        }
